...
def run_action_id(weapon):
    print(f'[{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] <Thread {weapon}> :: Getting connection to database')
    conn = pg8000.connect(
            host="localhost",
            port=54320,
            database="postgres",
    ...
    print(f'[{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] <Thread {weapon}> :: Parsing skin {weapon}')
...
skins = {
        ...
        'sniper_rifles': ['awp', 'g3sg1', 'scar-20', 'ssg-08'],
        ...
        'shot_guns': ['mag-7', 'nova', 'sawed-off', 'xm1014'],
        'machine_guns' : ['m249', 'negev'],
}

for type, items in skins.items():
    threads = []
    for i in items:
